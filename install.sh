...
configure() {
  ...
  ask "XIAOAI_DEFAULT_PROVIDER" "默认模型来源（openclaw/ollama/deepseek/openai/gemini）"
  ask "OPENCLAW_BASE_URL" "OpenClaw Bridge 地址（例如 http://设备IP:11435/v1）"
  ask_secret "OPENCLAW_API_KEY" "OpenClaw Bridge API Key"
  ask "OPENCLAW_DISPLAY_MODEL" "OpenClaw 语音显示模型名（默认 open）"
  ask "OLLAMA_BASE_URL" "Ollama 地址（例如 http://电脑IP:11434/v1，可留空）"
  ask "OLLAMA_MODEL" "Ollama 模型名"
  ask_secret "DEEPSEEK_API_KEY" "DeepSeek API Key（可留空）"
  ask "DEEPSEEK_MODEL" "DeepSeek 模型名"
  ask "OPENAI_BASE_URL" "OpenAI/兼容接口地址"
  ask_secret "OPENAI_API_KEY" "OpenAI/兼容接口 API Key（可留空）"
  ask "OPENAI_MODEL" "OpenAI/兼容接口模型名"
  ask_secret "GEMINI_API_KEY" "Gemini API Key（可留空）"
  ask "GEMINI_MODEL" "Gemini 模型名"
  ask "CONVERSATION_TURNS" "连续上下文轮数"
