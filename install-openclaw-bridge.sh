...
EOF
  ...
  echo "MiGPT 服务器 /opt/open-xiaoai-migpt/config.ts 里设置：openclaw.baseURL = \"http://${ip_addr}:${PORT}/v1\""
  if [ -n "$TOKEN" ]; then
    echo "MiGPT 服务器 /opt/open-xiaoai-migpt/config.ts 里设置：openclaw.apiKey = \"${TOKEN}\""
  else
