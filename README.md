...
AI 模式会启用 MiGPT 的连续对话保活，用静音/唤醒状态尽量压住原生小爱抢答；回到原生小爱模式后会退出保活。免刷机版默认把消息轮询和保活间隔调到 `500ms`，并内置静音 mp3 做保活；`开启AI` 默认不播欢迎语，尽量减少原生小爱开头抢答。需要自定义静音音频时，可在 `.env` 里配置音箱可访问的 `XIAOAI_AUDIO_SILENT` URL。
