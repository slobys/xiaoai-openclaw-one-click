...
默认 `PREFIX_GATE_MODE=text`，脚本会严格要求日志文本里带前缀；没说前缀的普通问题不会进入 AI。若某些设备会把前缀当唤醒词吃掉，导致日志里只剩问题正文，可改成 `PREFIX_GATE_MODE=keyword` 兼容模式，但这个模式依赖 MiGPT 关键词层拦截，可能放行未带前缀的问题。
